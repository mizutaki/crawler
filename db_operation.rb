...
item_nodes.each do |item|
  ...
  begin
    client.query("INSERT INTO residence(id, url, warking_time, rent, deposit, layout, azimuth, building_year, real_estate_company_name, real_estate_company_phonenumber) VALUES ('#{id}','#{link}', '#{warking_time}', '#{rent}', '#{deposit}', '#{layout}','#{azimuth}','#{building_year}', '#{real_estate_company_name}', '#{real_estate_company_phonenumber}')")
